...
//import the URL
const URL = 'https://api.themoviedb.org/3/tv/top_rated?language=en-US&page=1&with_watch_monetization_types=flatrate&sort_by=popularity.desc&include_adult=false&include_video=false';
...
const fetchTopSeries = createAsyncThunk("/topSeriesSlice", () => {
    return (
        fetch(URL, options).then(response => response.json()).then(data => data.results)
    )
})

export default fetchTopSeries;