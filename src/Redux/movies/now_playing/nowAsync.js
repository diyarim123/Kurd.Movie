...
//URL
const URL = 'https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=3&with_watch_monetization_types=flatrate&sort_by=popularity.desc&include_adult=false&include_video=false';
...
const fetchNowMovies = createAsyncThunk("/nowMMoviesSlice", () => {
    return (
        fetch(URL, options).then(response => response.json()).then(data => data.results)
    )
})

export default fetchNowMovies;